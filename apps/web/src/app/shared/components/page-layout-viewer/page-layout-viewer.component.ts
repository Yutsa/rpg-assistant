...
@Component({
  ...
  host: {
    class: 'page-layout-viewer-host',
    '[class.page-layout-viewer-host--embedded]': 'embedded()',
  },
})
export class PageLayoutViewerComponent {
  ...
  constructor() {
    combineLatest([toObservable(this.documentId), toObservable(this.pageNumber)])
      .pipe(
        tap(() => {
        ...
        switchMap(([documentId, pageNumber]) =>
          combineLatest([
            this.api.getPageMeta(documentId, pageNumber),
            this.api.listPageNodes(documentId, pageNumber),
          ]).pipe(
            map(([meta, nodes]) => ({
              meta,
              nodes,
              renderUrl: this.api.getPageRenderUrl(documentId, pageNumber, 120),
